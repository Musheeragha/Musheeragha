...
$modules = get-module | select -expand name
    ...
    { 
        ...
        {  
            ...
            { 
                $Props = @{Root=$Root;Type=$nestedmember.objectclass;Name=$nestedmember.name;DisplayName="";ParentGroup=$ADgroupname.name;Enabled="";Nesting=$nesting;DN=$nestedmember.distinguishedname;Comment=""} 
                ...
                { 
                    ...
                    { 
                    $table | select Root,type,name,parentgroup,nesting,displayname,enabled,dn,comment 
                    } 
                } 
                ...
                {  
                    ...
                    { 
                        ...
                        if(Select-String -Path .\UN_NestedMembers.csv -Pattern  "$([regex]::Escape($table."Root"))"",""group"",""$([regex]::Escape($table."name"))"",""$([regex]::Escape($table."parentgroup"))")
                        {
                            write-verbose $table."name"
                            $table."comment" = "Multilevel Circular Loop starting here"
                            $table | select Root,type,name,parentgroup,nesting,displayname,enabled,dn,comment
                            continue
                        } 
                    indent $table | select name,comment | %{
                                         ...
                                         write-output "$($_.name) (Single Level Circular Membership)"
                                         [console]::ResetColor()
                                         }
                                         else
                                         {
                    ...
                    { 
                       ...
                       Get-ADNestedGroupMembers -GroupName $nestedmember.distinguishedName -nesting $nesting -circular $circular -Root $Root -indent 
                    } 
                    else  
                    { 
                        Write-verbose $nestedmember.Name
                       
                                           
                       Get-ADNestedGroupMembers -GroupName $nestedmember.distinguishedName -nesting $nesting -circular $circular -Root $Root
                    } 
...
#Get-ADNestedGroupMembers "Nest1" -verbose | Export-CSV .\UN_NestedMembers.csv

$AllGroups = Get-ADGroup -Filter *
$Un_NestedGroups = @()
Foreach ($A in $AllGroups)
{
    $GroupMembers = Get-ADGroupMember $A.Samaccountname
    $SubgroupCount = 0
    Foreach ($GM in $GroupMembers)
    {        
        If ($GM.Objectclass -eq 'Group')
        {
            $SubgroupCount ++            
        }

    }
    if ($SubgroupCount -eq 0)
    {
       $Un_NestedGroups  += $A.samaccountname            
    }
}
foreach ($Group in $Un_NestedGroups )
{
    Get-ADNestedGroupMembers $Group -verbose | Export-CSV .\UN_NestedMembers.csv -append
}
